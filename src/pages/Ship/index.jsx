...
function Index() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery(["ship", id], () => fetchStarship(id), {
    enabled: true,
  });
  ...
  return (
    ship && (
      <div className="container  m-12 mx-auto w-4/5 flex-col md:flex-row border-4 shadow-2xl shadow-black border-yellow-500 flex">
        <motion.div
          ref={constraintsRef}
          className="flex flex-col w-full  bg-black justify-center  items-center border-yellow-500 "
        >
          <motion.img
            drag
        ...
        <div className="flex flex-col border-l-4 border-yellow-500 gap-3 w-full p-8 ">
          <h1
            className="text-center my-8 font-extrabold text-4xl         
        "
          >
            {ship.name}
          </h1>
          <h2 className="text-thin text-xl">
